...
- **Canvas Preview Invariant:** Irrespective of whether the active theme is Light, Dark, or System Auto, the QR preview container (`[data-tour="preview"]`) **must always preserve a high-contrast base** (`#ffffff` by default; shifts to `#0f172a` if dots are inverted to white). Never allow the canvas container to invert to dark in dark mode when dots are dark, because dark QR codes would become invisible. In `index.html` and `src/index.css`, `<meta name="color-scheme" content="light dark" />` and `color-scheme: only light` are enforced to prevent aggressive auto-dark algorithms (like Brave Mobile Night Mode) from inverting the white preview base.
...
- **Desktop Layout (`sm:flex`):** Renders a 3-button segmented pill (`Light`, `Dark`, `System`), direct action buttons for Scanner, Staff Mode, Batch CSV, Tour, and GitHub.
- **Mobile Layout (`sm:hidden`):** Renders a compact 34px floating dropdown for themes, an icon-only scanner button, and a **More Tools** overflow dropdown menu (`MoreVertical` button) exposing Staff Mode, Batch CSV Studio, Feature Tour, and GitHub audit links.
...
- **Cutout Technique:** Uses a transparent-center fixed overlay with CSS `box-shadow: 0 0 0 9999px rgba(2, 6, 23, 0.8)` and a glowing border ring. This avoids luminance mask inversion bugs found in SVG masks on Brave Night Mode, guaranteeing that highlighted controls remain 100% visible and bright.
